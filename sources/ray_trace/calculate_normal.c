...
/*   Updated: 2022/12/19 18:59:21 by pnoutere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_3d	cone_normal(t_object *cone, t_3d hit_point)
{
	...
	dp = (double)vector_magnitude(
			subtract_vectors(hit_point, cone->end)) / cos(dp);
...
static t_3d	cylinder_normal(t_object *cylinder, t_3d hit_point)
{
	...
	dp = dot_product(subtract_vectors(hit_point,
				cylinder->origin), cylinder->axis);
	normal = add_vectors(cylinder->origin,
			scale_vector(cylinder->axis, dp));
	normal = normalize_vector(subtract_vectors(hit_point, normal));
	return (normal);
}
...
static t_3d	box_normal(t_object *object, t_3d hit_point)
{
	...
	c = scale_vector(
			add_vectors(object->origin, object->end), 0.5);
