...
/*   Updated: 2022/12/19 21:19:54 by dmalesev         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	gradual_render(t_img *img, void *param)
{
	...
	env = param;
	if (env->scene->subframe.x == env->scene->subframe_range.x && env->scene->subframe.y == env->scene->subframe_range.y)
	{
		if (env->scene->accum_subframe.x == env->scene->subframe_range.x && env->scene->accum_subframe.y == env->scene->subframe_range.x)
			env->plot_time = (double)time_since_success(0.0f, 0,2);
	}
	if (env->scene->accum_subframe.x == env->scene->subframe_range.x && env->scene->accum_subframe.y == env->scene->subframe_range.y)
		env->plot_time = (double)time_since_success(0.0f, 0,2);
